namespace Hirabiki.Udon.Works
{
    ...
    public class PlayerLocalEffect : UdonSharpBehaviour
    {
        ...
        void Update()
        {
            ...
            if(nowDefeated != isDefeated && swimSystem.useCombatSystem)
            {
                isDefeated = nowDefeated;
                if(nowDefeated)
                {
                    oneShotSounds.PlayOneShot(deathSoundClip);
                    Physics.gravity = originalUnityGravity * 0.03f;
                } else
                {
                    swimSystem.WhenCombatRespawn();
                    Physics.gravity = originalUnityGravity;
                }
            }
            ...
            {
                if(lpf != null) lpf.enabled = !isDefeated && swimSystem.IsUnderwater();
            }

            if(!isDefeated && alertLowAir && Time.time > alertLowAirDelay)
            {
